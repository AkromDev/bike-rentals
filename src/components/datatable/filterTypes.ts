const stringFilterFn = (rows: any[], id: string, filter: Record<string, any>) => {
  let result = [];
  ...
  switch (operator) {
    case 'start':
      result = rows.filter((row) => {
        ...
        return rowValue
          ? String(rowValue).toLowerCase().startsWith(String(value).toLowerCase())
          : false;
      });
      break;
    case 'end':
...
stringFilterFn.autoRemove = (val: any) => !val;

const numberFilterFn = (rows: any[], id: string, filter: Record<string, any>) => {
  let result = [];
...
numberFilterFn.autoRemove = (val: any) => !val;

const dateFilterFn = (rows: any[], id: string, filter: Record<string, any>) => {
  let result = [];
  const { operator = 'eq', value } = filter;
  switch (operator) {
    case 'eq':
      result = rows.filter((row) => {
        const rowValue = row.values[id];
        return rowValue ? new Date(rowValue) === new Date(value) : false;
      });
      break;
    case 'not_eq':
      result = rows.filter((row) => {
        const rowValue = row.values[id];
        return rowValue ? new Date(rowValue) !== new Date(value) : true;
      });
      break;
    case 'gt':
      result = rows.filter((row) => {
...
dateFilterFn.autoRemove = (val: any) => !val;
